    ...
    def app_Header_Brand():
        return nlxFT.Container(
            content=nlxFT.Row(
                ...
                nlxFT.Text("Celeborn 0.0.6", size=15, color="white")
                ],
            ),
        )
...
Assets:
Status bar container added to the page as coppied from the said build.
Status value initialized in the "Value initializers".
Camera buttons for sit "Parking 1" installed.
Button format:
    nlxFT.ElevatedButton(
        text="<interface name>",
        bgcolor=nlxFT.colors.BLUE_GREY_100,
        color=nlxFT.colors.BLACK,
        expand=1,
        tooltip="<MAC Address Label>",
        data = {
            "IP": "<Camera IP Address>",
            "MAC": "<MAC Address>",
            "Interface": "<CSC Interface>",
        }
    )
Result: Good
References:
