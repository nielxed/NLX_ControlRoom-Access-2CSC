...
userComment = nlxFT.Text(value="", style=nlxFT.TextThemeStyle.TITLE_MEDIUM, weight=nlxFT.FontWeight.BOLD, color="red")
...
def main(page: nlxFT.Page):
    ...
    def open_nskBar_version(e):
        page.snack_bar = nlxFT.SnackBar(nlxFT.Text(f"Version: Rls. 0.2.6"))
    ...
    def login_dlg_UserLogin(e):
        ...
        if ssh_verifier.verify_ssh_access():
            ...
            userComment.value = f"{userNameCR}"
            page.update()
    ...
    def rbt_CSC_4108(e): # Test camera
        ...
        if inMain_loginChecker == 0:
            ...
            # setDesc4Cam_Global("Desmostheni Severi Parking 1 9OHQ.PRK1.6")
            # page.snack_bar = nlxFT.SnackBar(nlxFT.Text(f"Rebooting {currentCam.select}"))
            setHost4Cam_Global("192.168.201.4")
            setInt4Cam_Global("Gi1/0/8")
            open_dlg_modal_VER(e)
            ...
            content=nlxFT.Column(
                controls=[
                    ...
                    nlxFT.Container(
                        ...
                        content=
                            nlxFT.Row(
                                expand=True, height=30, alignment=nlxFT.MainAxisAlignment.START,
                                                    ...
                                                    controls=[
                                                        ...
                                                        nlxFT.Row(
                                                            controls=[
                                                                ...
                                                                nlxFT.ElevatedButton(text="GHQ.MEZ.46", tooltip="00:0F:7C:0E:E2:0D", bgcolor=nlxFT.colors.BLUE_GREY_100, color=nlxFT.colors.BLACK, width=175, on_click=rbt_CSC_111045)
                                                            ]
                                                        )
                                                    ]
                                                )
                                                    ...
                                                    controls=[
                                                        ...
                                                        nlxFT.Row(
                                                            ...
                                                            controls=[
                                                                ...
                                                                nlxFT.ElevatedButton(text="OHQ.PRK1.6", tooltip="3C:EF:8C:C5:06:0C", bgcolor=nlxFT.colors.BLUE_GREY_100, color=nlxFT.colors.BLACK, width=175, on_click=rbt_CSC_4108) # Test camera
                                                            ]
                                                        ),
                                                        # Row 2
                                                        nlxFT.Row(
                    ...
                    nlxFT.Container(
                        ...
                        content=nlxFT.Row(
                            ...
                            # self-control instances added here:
                            controls=[nlxFT.ElevatedButton(text="Actions list", bgcolor=nlxFT.colors.BLUE_GREY_100, color=nlxFT.colors.BLACK, expand=1, on_click=rbt_CSC_101024)]
                        )
                    ),
                    # nlxFT.Divider(height=1.5, color="Transparent"),
                    # Status bar panel called here
                    nlxFT.Container(
                        ...
                        bgcolor="#081d33", border_radius=nlxFT.border_radius.only(bottom_left=8, bottom_right=8), 
                            padding=nlxFT.padding.only(left=15, right=15),
                        content=nlxFT.Row(
                            expand=True, height=50, alignment=nlxFT.MainAxisAlignment.END,
                            # self-control instances added here:
                            controls=[status]
                        )
                    )
...
"""
Beta 1 Ver. of CRAcc2CSC Show Release
Assets:
None
New Assets:
Logging level added and set to INFO
Data class locks in asset data.
Triggering snack-bar when logo is clicked to reveal soft version.
Switch invalit username and password invalid access will trigger a banner instead of a model.
The banner has actions to cancel the CSC access process or retry.
Result: Good
References:
